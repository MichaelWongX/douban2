...
class DoubanmovieSpider(scrapy.Spider):
    ...
    def parse(self, response):
        if re.search('com/subject/\d+', response.url):
        ...
        for url in response.xpath('//a/@href').extract():
            ...
            if re.search('subject/\d+/\?', url) or re.search('subject/\d+/$', url) or 'tag' in url:
                yield self.gen_request(url)
    ...
    def parse_tag(self, response):
        ...
        for item in js['data']:
            yield self.gen_request(item['url'])
    ...
    def from_crawler(cls, crawler, *args, **kwargs):
        for key, value in crawler.settings.items():
            print(key,value)
    """
